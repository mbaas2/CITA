:Class CITARider
⍝ based on Nic's GhostRider - but focussed on the parts of RIDE that are relevant for CITA
⍝
    ...
    ∇ Constructor args;RIDE_INIT;_;env;host;port;r;runtime;tm1;tm2
      ...
      :If (0∊⍴args)∨(''≡0⍴args)  ⍝ spawn a local Ride - args is {env}
          ...
          :If 0∊⍴('\sDYAPP='⎕S 0)env  ⍝ don't inherit some environment variables
              env,←' DYAPP='
          :EndIf
          ⍝:If 0∊⍴('\sSESSION_FILE='⎕S 0)env  ⍝ don't inherit some environment variables:AndIf 1≤≢'FocusThread' 'SetPromptType'('Constructor'WaitFor 1)'CanAcceptInput'
          ⍝    env,←' SESSION_FILE='
          ⍝:EndIf
          ⎕←'CITARider: new APLProcess'
      ...
      :AndIf Send'["Identify",{"identity":1}]'  ⍝ 1=RIDE, 3=ProcessMgr - but 3 doesn't seem to differ...
      :AndIf 2≤≢'FocusThread' 'SetPromptType'('Constructor'WaitFor 1)'Identify' 'UpdateDisplayName' 'AppendSessionOutput'  ⍝ interpreter sends one 'Identify' and one 'UpdateDisplayName'
...
⍝       :AndIf EmptyQueue'SetPW'
    ...
    ⍝ Execution of a simple APL expression and get session output
      ...
      ⍝ :ElseIf wins≢NO_WIN ⋄ 'APL'Error'Expression produced a window: ',expr,', wins=',wins', prompt=',prompt
      :ElseIf wins≢NO_WIN ⋄ output,NL,NL,(80⍴'-'),NL,'APL-Expression produced a window: expr="',expr,'", wins=',(⎕json wins)', prompt=',prompt  ⍝ MB: don't care about this problem, just return whatever we got back (AND a msg...)
    ...
    ∇ R←SRCDIR
    :access public shared
      ⍝ source directory of current fn
           ⎕←'APLVersion=',4↑2⊃'.'⎕WG'APLVersion'
      :Select 4↑2⊃'.'⎕WG'APLVersion'
      :CaseList '18.0' '18.1'
          R←4⊃5179⌶1⊃⎕SI
      :Case '18.2'
          R←50 ⎕ATX 1⊃⎕SI
      :Else
      ⎕←'SRCDIR: no case defined for APLVersion=',4↑2⊃'.'⎕WG'APLVersion'
      :EndSelect
      R←1⊃⎕NPARTS R
    ∇
